---
name: review-plc
description: CODESYS ST 코드 리뷰. Bitbucket PR의 PLC 코드를 Infrastructure 아키텍처, 네이밍 규칙, 시퀀스 데드락, 모듈화 관점으로 분석. 사용법: /review-plc <PR_URL>
...
Bitbucket PR URL을 입력받아 CODESYS Structured Text 코드를 PLC 도메인 전문 관점으로 분석하고, 구조화된 리뷰 코멘트를 PR에 자동으로 게시합니다.
...
```bash
python3 .claude/skills/review-pr/scripts/review_pr.py fetch <pr_url>
```
...
```bash
python3 .claude/skills/review-pr/scripts/review_pr.py comment <pr_url> < /tmp/review_pr_{id}.md
```
...
1. 파일명 정보 조회:
```bash
python3 .claude/skills/review-pr/scripts/review_pr.py save <pr_url>
```
